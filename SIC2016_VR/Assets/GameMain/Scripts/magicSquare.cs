...
public class magicSquare : MonoBehaviour
{
    ...
    void Emit()
    {
        ...
        if (emit != null)
        {
            if (Physics.BoxCast(transform.position,new Vector3(transform.lossyScale.x,transform.lossyScale.y,1.0f),transform.forward,out Hit,transform.rotation,10000,1 << LayerMask.NameToLayer("enemy")))
            {
